...
async function demonstrateExplainBasics(client) {
  ...
  await runQuery(client, 'EXPLAIN SELECT 1', 'Simple EXPLAIN - shows basic execution plan');
...
async function demonstrateErrorHandling(client) {
  ...
  await runQuery(client, 'EXPLAIN', 'Missing query - should return error');
}
...
async function main() {
  console.log(
    colorize('PostgreSQL Wire Protocol Mock Server - EXPLAIN Demo', colors.bright + colors.green)
  );
  ...
  try {
    ...
    console.error(
      colorize('Make sure the mock server is running on localhost:5432', colors.yellow)
    );
    process.exit(1);
  } finally {
    await client.end();
