...
const { MESSAGE_TYPES, ERROR_CODES, AUTH_METHODS, TRANSACTION_STATUS } = require('../protocol/constants');
...
class ProtocolTester {
   ...
   */
  async runComplianceTests(options = {}) {
    return await this.runTestCategory('compliance', options);
  }
  ...
  async runEdgeCaseTests(options = {}) {
    return await this.runTestCategory('edgeCases', options);
  }
   ...
   * @returns {Promise<Object>} Test results
   */
  async runErrorConditionTests(options = {}) {
    return await this.runTestCategory('errorConditions', options);
  }
  ...
  // Compliance Tests
  async testStartupProtocolVersion(options = {}) {
  ...
  async testAuthenticationFlow(options = {}) {
    ...
    try {
      ...
      const saslMessage = this.createSASLMessage(['SCRAM-SHA-256']);
      result.details.saslMessage = 'PASSED';
      
      // Test SASL continue
      const saslContinue = this.createSASLContinueMessage('r=test,s=test,i=4096');
      result.details.saslContinue = 'PASSED';
      
      // Test SASL final
      const saslFinal = this.createSASLFinalMessage('v=test');
  ...
  async testQueryExecutionFlow(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test simple query
      const query = this.createQueryMessage('SELECT 1');
      result.details.simpleQuery = 'PASSED';
      
      // Test query with parameters
      const paramQuery = this.createQueryMessage('SELECT $1, $2');
      result.details.parameterizedQuery = 'PASSED';
      
      // Test empty query
      const emptyQuery = this.createQueryMessage('');
  ...
  async testExtendedQueryProtocol(options = {}) {
    ...
    try {
      // Test Parse message
      const parseMessage = this.createParseMessage('stmt1', 'SELECT $1', [23]);
      result.details.parseMessage = 'PASSED';
      
      // Test Bind message
      const bindMessage = this.createBindMessage('portal1', 'stmt1', ['test']);
      result.details.bindMessage = 'PASSED';
      
      // Test Describe message
      const describeMessage = this.createDescribeMessage('S', 'stmt1');
      result.details.describeMessage = 'PASSED';
      
      // Test Execute message
      const executeMessage = this.createExecuteMessage('portal1', 0);
  ...
  async testTransactionStateManagement(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    ...
    try {
      ...
      // Test initial state
      if (connState.transactionStatus !== TRANSACTION_STATUS.IDLE) {
        result.passed = false;
      ...
      if (connState.transactionStatus !== TRANSACTION_STATUS.IN_TRANSACTION) {
        result.passed = false;
  ...
  async testErrorResponseFormat(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test error response with all fields
      const errorResponse = this.createErrorResponse(ERROR_CODES.SYNTAX_ERROR, 'Test error', {
      ...
      const minimalError = this.createErrorResponse(ERROR_CODES.INTERNAL_ERROR, 'Internal error');
  ...
  async testParameterStatusMessages(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test parameter status message
      const paramStatus = this.createParameterStatusMessage('server_version', '13.0');
      ...
      for (const param of params) {
        const paramMsg = this.createParameterStatusMessage(param.name, param.value);
  ...
  async testBackendKeyData(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test backend key data message
      const backendKey = this.createBackendKeyDataMessage(12345, 67890);
      result.details.backendKeyData = 'PASSED';
      
      // Test with zero values
      const zeroKey = this.createBackendKeyDataMessage(0, 0);
  ...
  // Edge Case Tests
  async testEmptyQueryString(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    ...
    try {
      ...
      const whitespaceQuery = this.createQueryMessage('   ');
  ...
  async testVeryLongQuery(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test query at maximum length
      const longQuery = 'SELECT ' + '1, '.repeat(10000) + '1';
      const longQueryMessage = this.createQueryMessage(longQuery);
      result.details.veryLongQuery = 'PASSED';
      
      // Test query with maximum parameter count
      const paramQuery = 'SELECT ' + Array(1000).fill('$1').join(', ');
      const paramQueryMessage = this.createQueryMessage(paramQuery);
  ...
  async testUnicodeCharacters(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test unicode in query
      const unicodeQuery = this.createQueryMessage('SELECT \'你好世界\' as greeting');
      result.details.unicodeQuery = 'PASSED';
      
      // Test emoji in query
      const emojiQuery = this.createQueryMessage('SELECT \'🚀 Database 🎉\' as message');
      result.details.emojiQuery = 'PASSED';
      
      // Test mixed unicode
      const mixedQuery = this.createQueryMessage('SELECT \'Hello 世界 🌍\' as mixed');
  ...
  async testSpecialCharacters(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test special characters in query
      const specialQuery = this.createQueryMessage('SELECT \'test\\n\\r\\t\\0\' as special');
      result.details.specialCharacters = 'PASSED';
      
      // Test SQL injection attempts
      const injectionQuery = this.createQueryMessage('SELECT * FROM users WHERE id = 1; DROP TABLE users;');
  ...
  async testLargeResultSets(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    ...
    try {
      ...
      const rowDesc = this.createRowDescriptionMessage(manyColumns);
      result.details.manyColumns = 'PASSED';
      
      // Test data row with many fields
      const manyFields = Array(100).fill('test_value');
      const dataRow = this.createDataRowMessage(manyFields);
  ...
  async testManyColumns(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test maximum number of columns
      const maxColumns = Array(1600).fill(null).map((_, i) => ({
        name: `column_${i}`,
        dataTypeOID: 25,
        dataTypeSize: -1
      }));
      
      const rowDesc = this.createRowDescriptionMessage(maxColumns);
  ...
  async testInvalidMessageType(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
  ...
  async testMalformedMessageLength(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
  ...
  async testIncompleteMessages(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
  ...
  async testInvalidAuthentication(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
  ...
  async testProtocolViolations(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test message out of sequence
      const executeWithoutParse = this.createExecuteMessage('portal1', 0);
      result.details.executeWithoutParse = 'PASSED';
      
      // Test bind without parse
      const bindWithoutParse = this.createBindMessage('portal1', 'nonexistent', []);
  ...
  async testConnectionStateErrors(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
  ...
  async testMemoryExhaustion(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
  ...
  async testTimeoutConditions(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test slow message processing
      const slowMessage = this.createQueryMessage('SELECT pg_sleep(1)');
  ...
  createParseMessage(statementName, query, paramTypes = []) {
    ...
    const buffer = Buffer.alloc(1 + 4 + stmtNameBuffer.length + queryBuffer.length + 2 + paramTypesBuffer.length);
  ...
  createBindMessage(portalName, statementName, parameters = []) {
    ...
    const length = portalBuffer.length + stmtBuffer.length + 4 + paramFormats.length + 2 + paramValues.reduce((sum, buf) => sum + buf.length, 0);
    const buffer = Buffer.alloc(1 + 4 + portalBuffer.length + stmtBuffer.length + 4 + paramFormats.length + 2 + paramValues.reduce((sum, buf) => sum + buf.length, 0));
  ...
  async testBufferOverflow(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
  ...
  async testInvalidDataTypes(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test with invalid data type OID
      const invalidOID = 999999;
      const rowDesc = this.createRowDescriptionMessage([{
        name: 'test',
      ...
      const negativeOID = -1;
      const negativeRowDesc = this.createRowDescriptionMessage([{
        name: 'test',
  ...
  async testMalformedArrays(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test malformed array syntax
      const malformedArray = this.createQueryMessage("SELECT '{1,2,3'::int[]");
      result.details.malformedArray = 'PASSED';
      
      // Test array with invalid dimensions
      const invalidDimensions = this.createQueryMessage("SELECT '{{1,2},{3}}'::int[][]");
      result.details.invalidDimensions = 'PASSED';
      
      // Test array with mixed types
      const mixedTypes = this.createQueryMessage("SELECT '{1,2.5,3}'::int[]");
  ...
  async testInvalidUTF8Sequences(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
    
    try {
      // Test invalid UTF-8 sequences
      const invalidUTF8 = Buffer.from([0xFF, 0xFE, 0xFD, 0xFC]);
      const queryWithInvalidUTF8 = this.createQueryMessage(invalidUTF8.toString('binary'));
      ...
      const queryWithIncompleteUTF8 = this.createQueryMessage(incompleteUTF8.toString('binary'));
      ...
      const queryWithOverlongUTF8 = this.createQueryMessage(overlongUTF8.toString('binary'));
  ...
  async testConcurrentConnectionLimits(options = {}) {
    const result = { passed: true, warnings: [], details: {} };
